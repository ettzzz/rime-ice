...
copy all yaml files into :

- Windows: `~\AppData\Roaming\Rime` # weasel.custom.yaml works
- Mac: `~/Library/Rime` # squirrel.custom.yaml works
- Linux: `~/.config/ibus/rime`, # tested on ubuntu, but it's recommended to use `sudo apt install chrome-gnome-shell` with `Customize Ibus` and `plum` to manage style
